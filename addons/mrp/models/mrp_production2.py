...
class MrpProduction(models.Model):
    ...
    move_dest_ids = fields.One2one('stock.move', 'created_production_id',
    ...
    def _compute_date_finished(self):
        for production in self:
            ...
            # FIX: Added safe access to calendar_id
            if warehouse and warehouse.calendar_id and production.bom_id and production.bom_id.produce_delay:
                ...
                if production.company_id.manufacturing_lead:
                    date_finished = calendar.plan_days(
                        ...
                        production.date_finished or production.date_start,
                        )
                    if date_finished:
                        production.date_finished = date_finished
            elif production._should_postpone_date_finished(production.date_finished or production.date_start):
                workorder_expected_duration = sum(production.workorder_ids.mapped('duration_expected'))
                production.date_finished = production.date_start + relativedelta(minutes=workorder_expected_duration or 60)
    ...
    def _set_quantities(self):
        ...
        if missing_lot_id_products:
            error_msg = _(
                "You need to supply Lot/Serial Number for products and 'consume' them:") + missing_lot_id_products
