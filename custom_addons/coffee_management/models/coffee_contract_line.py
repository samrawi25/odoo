...
class CoffeeContractLine(models.Model):
    ...
    quantity_kg = fields.Float(
        string='Quantity (KG)', compute='_compute_quantity_kg', store=True
    )
    unit_price_usd_per_lb = fields.Float(
        string='Unit Price (USD/LB)', required=True, digits='Product Price'
    )
    price_per_kg = fields.Float(
        string='Unit Price (USD/KG)', compute='_compute_price_per_kg', store=True
    )
    subtotal_usd = fields.Float(
        string='Subtotal (USD)', compute='_compute_subtotal_usd', store=True
    )
    state = fields.Selection(
        related='contract_id.state', string='Contract Status', store=True, readonly=True
    )
