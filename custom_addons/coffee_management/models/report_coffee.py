...
class ReportDateRangeWizard(models.TransientModel):
    ...
    # Add security check methods
    @api.model
    def check_access_rights(self, operation, raise_exception=True):
        """Override to add custom security checks for reports"""
        if operation in ['write', 'create', 'unlink']:
            if not self.env.user.has_group('coffee_management.group_coffee_admin') and \
                    not self.env.user.has_group('coffee_management.group_coffee_report_manager'):
                if raise_exception:
                    raise AccessError("You don't have permission to modify reports.")
                return False
    ...
    def check_report(self):
        ...
        # Security check - ensure user has permission to access reports
        if not self._check_report_permissions():
            raise UserError("You don't have permission to generate this report.")
    ...
    def _check_report_permissions(self):
        """Check if user has permission to access reports"""
        user = self.env.user
        return (user.has_group('coffee_management.group_coffee_admin') or
                user.has_group('coffee_management.group_coffee_report_manager') or
                user.has_group('coffee_management.group_coffee_report_viewer'))
...
class CoffeeArrivalQualityReport(models.AbstractModel):
    ...
    def _get_report_data(self, date_from, date_to):
        ...
        Fetches and processes data for the Arrival and Quality Report.
        """
        # Security check - ensure user has permission to access this data
        if not self.env.user.has_group('coffee_management.group_coffee_admin') and \
                not self.env.user.has_group('coffee_management.group_coffee_report_manager') and \
                not self.env.user.has_group('coffee_management.group_coffee_report_viewer'):
            return {
                'arrivals': [],
                'summary': {'total_arrivals': 0, 'total_kg': 0.0, 'avg_moisture': 0.0, 'avg_score': 0.0,
                            'low_grade_count': 0},
                'access_denied': True
            }
...
class CoffeeStockMovementReport(models.AbstractModel):
    ...
    def _get_report_data(self, date_from, date_to):
        ...
        if not self.env.user.has_group('coffee_management.group_coffee_admin') and \
                not self.env.user.has_group('coffee_management.group_coffee_report_manager') and \
                not self.env.user.has_group('coffee_management.group_coffee_report_viewer'):
            return {'stock_summary': [], 'access_denied': True}
...
class CoffeeContractFulfillmentReport(models.AbstractModel):
    ...
    def _get_report_data(self, date_from, date_to):
        ...
        if not self.env.user.has_group('coffee_management.group_coffee_admin') and \
            ...
            return {
                'contracts': [],
                'summary': {'total_contracts': 0, 'total_ordered_kg': 0.0, 'total_delivered_kg': 0.0},
                'access_denied': True
            }
